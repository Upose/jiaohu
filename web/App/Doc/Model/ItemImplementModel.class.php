...
class ItemImplementModel
{
     ...
     * 2018.01.11
     */
    public function pWeeklyAdd($pro_code,$weekly_code,$weekly_name,$plan_code,$weekly_stime,$weekly_etime,$stage,$pro_schedule,$founder_id,$upWeeklyList,$weeklyList,$nextWeeklyList){
        ...
        if ($weekly_code) {
            ...
            if ($weeklyList) {
                ...
                //插入本周报任务
                $data['pro_code'] = $pro_code;
                $data['weekly_name'] = $this1.'_'.$this7;
                $data['plan_code'] = $plan_code;
                $data['weekly_stime'] = $this1;
                $data['weekly_etime'] = $this7;
                $data['stage'] = $stage;
                $data['pro_schedule'] = $pro_schedule;
                $data['founder_id'] = $founder_id;
                $weeklyID1 = $app_project_weekly->add($data);
                if ( !empty($weeklyID1) && !empty($weeklyList)) {
                        for ($i=0; $i < count($weeklyList); $i++) { 
                           
                                 //插入周报任务明细
                                $detailed['weekly_id'] = $weeklyID1;
                                $detailed['pro_code'] = $pro_code;
                                $detailed['completion_rate'] = $weeklyList[$i]['cRate'];
                                $detailed['work_content'] = $weeklyList[$i]['wContent'];
                                $detailed['work_state'] = $weeklyList[$i]['wState'];
                ...
                //插入下周报任务
                $data['pro_code'] = $pro_code;
                $data['weekly_name'] = $next1.'_'.$next7;
                $data['plan_code'] = $plan_code;
                $data['weekly_stime'] = $next1;
                $data['weekly_etime'] = $next7;
                $data['stage'] = $stage;
                $data['pro_schedule'] = $pro_schedule;
                $data['founder_id'] = $founder_id;
                $weeklyID2 = $app_project_weekly->add($data);
                ...
                if ( !empty($weeklyID2) && !empty($nextWeeklyList)) {
                        for ($i=0; $i < count($nextWeeklyList); $i++) { 
                                 ...
                                 //插入周报任务明细
                                $detailed['weekly_id'] = $weeklyID2;
                                $detailed['pro_code'] = $pro_code;
                                $detailed['completion_rate'] = $nextWeeklyList[$i]['cRate'];
                                $detailed['work_content'] = $nextWeeklyList[$i]['wContent'];
                                $detailed['work_state'] = $nextWeeklyList[$i]['wState'];
                                $detailed['remarks'] = $nextWeeklyList[$i]['remarks'];
                                $detailed['founder_id'] = $founder_id;
                                $weeklyDetailedResult = $app_weekly_task->add($detailed);
    ...
    private function selectTimesWeek($code){
